...
if(isset($_POST["chatGptText"])){
  ...
  $postTitle = 'Your ' . $_POST['chatGptText'] . ' Reading';   //adding prompt to Blog Title field
  $postKeywords = $_POST['blogKeywordText'];  //adding prompt to Blog Keyword field
  
  $prompt = CHATGPTTOO()->helpers->get_astrology_prompt( $_POST["chatGptText"], $_POST["dateOfBirthText"], $_POST["timeOfBirth"], $_POST["birthPlace"]);
  //console_log($prompt);
