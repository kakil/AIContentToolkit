...
if (isset($_POST["chatGptText"])) {
  ...
  $prompt = AICONTENTT()->helpers->get_short_story_prompt($topic);
...
<div class="container-fluid m-3 w-50 bg-light border border-3 shadow p-3 rounded-4"> 
  <h1 class="mt-5 mb-5">AI Short Story Tool</h1>
  <form method="post" id="blogPostForm"  class="needs-validation" novalidate>
    
    <!-- Product 1 -->
    <div class="mb-3">
      <label for="validationCustom2001" class="form-label">
        <?php echo $lang["shortStoryTopic"]; ?>
      </label>
      <input type="text" class="form-control" id="validationCustom2001" name="chatGptText"
        placeholder='"pyramids, ufo, starry night", "best friends, college roommates, math"' min="0" max="80" value="<?php echo isset($_POST['chatGptText']) ? $_POST['chatGptText'] : '' ?>" required>
    ...
    <div class="row mb-5">
       <div class="col-sm-9">
         <button type="submit" name="goTest" class="btn btn-primary mb-3" id="btn-submit"><?php echo $lang["shortStoryButton"]; ?>
