...
if (isset($_POST["chatGptText"])) {
  ...
  $prompt = AICONTENTT()->helpers->get_inspirational_quotes_prompt($topic);
...
<div class="container-fluid m-3 w-50 bg-light border border-3 shadow p-3 rounded-4"> 
  <h1 class="mt-5 mb-5">AI Inspirational Quotes Tool</h1>
  <form method="post" id="blogPostForm"  class="needs-validation" novalidate>
    
    <!-- Product 1 -->
    <div class="mb-3">
      <label for="validationCustom2301" class="form-label">
        <?php echo $lang["shortStoryTopic"]; ?>
      </label>
      <input type="text" class="form-control" id="validationCustom2301" name="chatGptText"
        placeholder="Confidence" min="0" max="80" value="<?php echo isset($_POST['chatGptText']) ? $_POST['chatGptText'] : '' ?>" required>
    ...
    <div class="row mb-5">
       <div class="col-sm-9">
         <button type="submit" name="goTest" class="btn btn-primary mb-3" id="btn-submit"><?php echo $lang["quotesButton"]; ?>
  ...
  <form method="POST" id="blogForm" class="needs-validation" novalidate>
    ...
    <div class="mb-3">
      ...
      <input type="text" name="postTitle" placeholder="Confidence Quotes" id="postTitle"
